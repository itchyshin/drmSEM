...
if (!requireNamespace("drmTMB", quietly = TRUE)) {
  stop("inst/validation/generate.R needs the drmTMB engine (github::itchyshin/drmTMB).")
}
...
run_c1 <- function(seed, n, B, level) {
  empty <- function(status) {
    data.frame(
      ...
      truth = NA_real_, estimate = NA_real_,
      conf.low = NA_real_, conf.high = NA_real_,
      covered = NA, width = NA_real_, status = status,
      stringsAsFactors = FALSE
    )
  }
  built <- tryCatch(suppressMessages(make_c1(seed, n)), error = function(e) NULL)
  if (is.null(built)) return(empty("build_failed"))

  tot <- tryCatch(
    suppressMessages(total_effects(
      built$sem, from = "x", to = "y", method = "simulate",
      uncertainty = "parametric", B = B, level = level, seed = seed
    )),
    error = function(e) NULL
  )
  dec <- tryCatch(
    suppressMessages(indirect_effects(
      built$sem, from = "x", to = "y", effect = "controlled",
      uncertainty = "parametric", B = B, level = level, seed = seed
    )),
    error = function(e) NULL
  )
  if (is.null(tot) || is.null(dec)) return(empty("fit_failed"))
  ...
  data.frame(
    ...
    truth = rows$truth, estimate = rows$estimate,
    conf.low = rows$conf.low, conf.high = rows$conf.high,
    covered = rows$covered, width = rows$width, status = "ok",
    stringsAsFactors = FALSE
  )
}
...
# Over replicates we record best()'s selection and the Akaike weight on truth.
# ----------------------------------------------------------------------------
...
run_c3 <- function(seed, n) {
  empty <- function(status) data.frame(
    n = n, seed = seed, selected = NA_character_,
    selected_truth = NA, truth_weight = NA_real_, status = status,
    stringsAsFactors = FALSE
  )
  dat <- tryCatch(make_c3_data(seed, n), error = function(e) NULL)
  if (is.null(dat)) return(empty("build_failed"))
  cmp <- tryCatch(
    suppressMessages(compare(
      c3_model_set(), data = dat,
      family = list(m = stats::gaussian(), y = stats::gaussian())
    )),
    error = function(e) NULL
  )
  if (is.null(cmp)) return(empty("compare_failed"))

  selected <- cmp$model[[which.min(cmp$CICc)]]
  truth_weight <- {
    w <- cmp$weight[cmp$model == "truth"]
...
# ----------------------------------------------------------------------------
message(
  "drmSEM wave-2 C-1 coverage: ", length(GRID$n_grid), " n x ", GRID$R,
  " reps = ", length(GRID$n_grid) * GRID$R, " fits (B = ", GRID$B, ")."
)
...
# ----------------------------------------------------------------------------
message(
  "drmSEM wave-2 C-3 model selection: ", length(GRID$n_grid), " n x ", GRID$R,
  " reps = ", length(GRID$n_grid) * GRID$R, " comparisons."
)
...
# ----------------------------------------------------------------------------
ok1 <- c1_per_rep[c1_per_rep$status == "ok" & !is.na(c1_per_rep$covered), ,
                  drop = FALSE]
...
coverage <- {
  ...
  out <- merge(merge(cov_rate, n_rep, by = c("n", "quantity")),
               width, by = c("n", "quantity"))
  out
}
...
# ----------------------------------------------------------------------------
# C-3 distillation: selection rate of the truth + mean Akaike weight on truth,
# per n. Acceptance: selection rate is high (>= 0.80) and the missing-edge rival
# is essentially never selected.
# ----------------------------------------------------------------------------
ok3 <- c3_per_rep[c3_per_rep$status == "ok" & !is.na(c3_per_rep$selected_truth), ,
                  drop = FALSE]

model_selection <- {
  by <- list(n = ok3$n)
  sel  <- aggregate(list(selection_rate = ok3$selected_truth), by = by, FUN = mean)
  wt   <- aggregate(list(mean_truth_weight = ok3$truth_weight), by = by,
                    FUN = function(z) mean(z, na.rm = TRUE))
  miss <- aggregate(list(missing_edge_rate = ok3$selected == "missing_edge"),
                    by = by, FUN = mean)
  nrep <- aggregate(list(n_rep = ok3$selected_truth), by = by, FUN = length)
  out <- merge(merge(merge(sel, wt, by = "n"), miss, by = "n"), nrep, by = "n")
  out
}
model_selection$acceptance <- model_selection$selection_rate >= 0.80 &
  model_selection$missing_edge_rate <= 0.05
model_selection <- model_selection[order(model_selection$n), , drop = FALSE]
rownames(model_selection) <- NULL

# Selection rate should be nondecreasing in n (a secondary diagnostic).
selection_monotone <-
  all(diff(model_selection$selection_rate[order(model_selection$n)]) >= -0.05)
...
acceptance <- list(
  ...
  model_selection  = all(model_selection$acceptance),
  selection_monotone = selection_monotone,
  criteria = data.frame(
    criterion = c(
      "C1 effect-CI coverage",
      "C3 model-selection recovery",
      "C3 selection rate increasing in n"
    ),
    rule = c(
      "Every (n, quantity) empirical coverage lies inside nominal +/- 2*SE.",
      "Every n has truth selection rate >= 0.80 and missing-edge selection rate <= 0.05.",
      "Truth selection rate is nondecreasing in n (within 0.05 Monte-Carlo jitter)."
    ),
    pass = c(
      all(coverage$acceptance),
      all(model_selection$acceptance),
      selection_monotone
    ),
    stringsAsFactors = FALSE
  )
...
message(
  "Wrote ", out_path, " (drmTMB ", provenance$drmTMB_version,
  ", drmTMB sha ", provenance$drmTMB_remote_sha,
  ", drmSEM sha ", provenance$git_sha, ")."
)
