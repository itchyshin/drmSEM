...
test_that("family samplers recover their target moments", {
  ...
  g <- drm_sample_family(
    "gaussian",
    list(mu = rep(3, 1e4), sigma = rep(2, 1e4)),
    1e4
  )
  ...
  zi <- drm_sample_family(
    "poisson",
    list(mu = rep(5, 1e4), zi = rep(0.5, 1e4)),
    1e4
  )
...
test_that("a mediator's scale propagates only under distribution mediation", {
  ...
  contrast <- function(eng, mediation, n_sim) {
    mean(
      drm_expected_target(eng, hi, "Y", "M", mediation, NULL, n_sim) -
  ...
  eng0 <- make_engines(0)
  expect_equal(
    contrast(eng0, "distribution", 400),
    contrast(eng0, "mean", 1),
    tolerance = 0.2
  )
  # scale increases with x: distribution mediation adds a positive effect
  eng1 <- make_engines(0.9)
  expect_gt(
    contrast(eng1, "distribution", 800) - contrast(eng1, "mean", 1),
    0.1
  )
})
...
test_that("V-76: analytic outcome functionals match closed forms", {
  ...
  expect_equal(
    drmSEM:::drm_analytic_functional("gaussian", g, "quantile", prob = 0.975),
    stats::qnorm(0.975, c(1, 4), c(2, 3))
  )
  expect_equal(
    drmSEM:::drm_analytic_functional("gaussian", g, "p_gt", threshold = 0),
    stats::pnorm(0, c(1, 4), c(2, 3), lower.tail = FALSE)
  )
  expect_equal(
    drmSEM:::drm_analytic_functional("gaussian", g, "p_zero"),
    c(0, 0)
  )

  p <- data.frame(mu = c(0.5, 2))
  expect_equal(
    drmSEM:::drm_analytic_functional("poisson", p, "p_zero"),
    exp(-c(0.5, 2))
  )
