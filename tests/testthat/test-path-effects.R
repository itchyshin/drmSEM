...
pe_scen <- function() {
  list(
    lo = pe_rows(data.frame(x = 0, M1 = 0, M2 = 0, Y = 0), 20),
    hi = pe_rows(data.frame(x = 1, M1 = 0, M2 = 0, Y = 0), 20),
    column = "x"
  )
}
pc_mean <- function(eng, meds) {
  drmSEM:::drm_path_contrasts(
    eng,
    pe_scen(),
    "Y",
    meds,
...
test_that("by-component: mean channel is exact, distributional channel matches the lognormal form", {
  ...
  scen <- list(
    lo = pe_rows(data.frame(x = x0, M = 0, Y = 0), 120),
    hi = pe_rows(data.frame(x = x1, M = 0, Y = 0), 120),
    column = "x"
  )
  run <- function(s1) {
    eng <- list(
  ...
  expect_equal(
    mean(pc$mean_inclusion$M),
    exp(k * a * x1) - exp(k * a * x0),
    tolerance = 1e-8
  )
  ...
  expect_equal(
    mean(pc$mean_inclusion$M) + dist_ch,
    mean(pc$inclusion$M),
    tolerance = 1e-12
  )
  ...
  pc_lin <- drmSEM:::drm_path_contrasts(
    eng_lin,
    scen,
    "Y",
    "M",
  ...
  expect_equal(
    mean(pc_lin$inclusion$M - pc_lin$mean_inclusion$M),
    0,
    tolerance = 0.02
  )
})
...
test_that("by-component freeze attributes the sigma channel (OQ-5 per-component)", {
  ...
  cc <- drmSEM:::drm_component_contrasts(
    ms_engine(0.9),
    scen,
    "Y",
    "M",
  ...
  expect_equal(
    mean(cc$mean) + mean(cc$channels$sigma) + mean(cc$remainder),
    mean(cc$inclusion),
    tolerance = 1e-12
  )
  # negative control: a flat scale path (s1 = 0) -> sigma channel is exactly 0
  cc0 <- drmSEM:::drm_component_contrasts(
    ms_engine(0),
    scen,
    "Y",
    "M",
...
test_that("recanting-witness detection flags sequential mediators (OQ-5 natural)", {
  ...
  expect_false(drmSEM:::drm_recanting_witness(
    obj_par,
    "x",
    "m1",
    c("m1", "m2")
