...
test_that("drm_build_covariances resolves nodes, labels, and de-duplicates", {
  ...
  one <- drmSEM:::drm_build_covariances(covary("activity", "boldness"), records3)
  ...
  hl <- drmSEM:::drm_build_covariances(
    covary("activity", "boldness", level = "id"), records3
  )
  ...
  dup <- drmSEM:::drm_build_covariances(
    list(covary("activity", "boldness"), covary("boldness", "activity")), records3
  )
...
test_that("a declared covariance edge drops the y1 _||_ y2 independence claim", {
  ...
  cov_df <- data.frame(
    y1 = "y1", y2 = "y2", class = "residual", level = NA_character_,
    structure = "unstructured", label = "rho12(y1, y2)", stringsAsFactors = FALSE
  )
  ...
  cov_hl <- data.frame(
    y1 = "y2", y2 = "y1", class = "higher_level", level = "id",
    structure = "unstructured", label = "corpair(id: y2, y1)", stringsAsFactors = FALSE
  )
  bs_hl <- basis_set(make_sibling_obj(cov_hl))
  expect_false(any((bs_hl$x == "y1" & bs_hl$y == "y2") |
