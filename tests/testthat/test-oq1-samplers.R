...
node_params <- function(fit, family_name) {
  ...
  mu <- drm_inv_link(
    drm_nominal_link(family_name, "mu"),
    unname(mu_b[["(Intercept)"]])
  )
...
expect_sampler_recovers <- function(
  family_name,
  fit,
  y,
  N = 2e5,
  ...
  draws <- drm_sample_family(
    family_name,
    list(mu = rep(pr$mu, N), sigma = rep(pr$sigma, N)),
    N
  )
  dm <- mean(y)
  dv <- stats::var(y)
  ...
  rv <- stats::var(draws)
  info <- sprintf(
    "%s: sampler(mean=%.4g,var=%.4g) vs data(mean=%.4g,var=%.4g) [mu=%.4g,sigma=%.4g]",
    family_name,
    rm,
    rv,
    dm,
